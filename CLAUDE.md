...
Before any Playwright MCP call or `npm run reload`, run `npm run ensure-dev`. It's idempotent: if the dev server is already up it exits immediately; if not, it spawns `npm start` detached (logs to `.vite/dev.log`) and blocks until the new boot marker lands. Prefer this over asking the user to start npm themselves.
