...
test.describe('agent: real Claude round-trip', () => {
  ...
  test('sends a prompt and streams a response back into the transcript', async () => {
    ...
    await expect(assistantBubble).toHaveAttribute('data-streaming', 'false', {
      timeout: 120_000,
    });

    // The response parses as the expected JSON shape.
    const raw = (await assistantBubble.locator('.bubble-text').textContent()) ?? '';
    const jsonMatch = raw.match(/\{[\s\S]*\}/);
    expect(jsonMatch, `response did not contain JSON: ${raw}`).not.toBeNull();
    const parsed = JSON.parse(jsonMatch![0]) as { status?: string; echo?: string };
