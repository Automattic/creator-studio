...
export class AgentService {
  ...
  async send(prompt: string): Promise<void> {
    ...
    if (!apiKey) {
      this.emit({ kind: 'error', message: 'ANTHROPIC_API_KEY is not set' });
    ...
    switch (msg.type) {
      ...
      case 'stream_event': {
        ...
        if (
          ...
          this.emit({ kind: 'tool-use', toolName: ev.content_block.name });
        }
        return;
      }

      case 'result':
        this.emit({ kind: 'done', success: msg.subtype === 'success' });
        return;
    }
  }
